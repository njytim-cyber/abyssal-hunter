 ...
 */
const HUD = memo(function HUD({ score, rank, inkPercent, combo, comboMultiplier, muted, onMuteToggle }: HUDProps) {
    return (
        <div className="hud">
            <h1>Abyssal Hunter</h1>
            <div className="stat">MASS: <span id="score">{score}</span></div>
            <div className="stat rank-display">RANK: <span id="rank">{rank}</span></div>
            ...
            <div className="controls-hint">
                WASD/Arrows to move • Space/Click to dash
            </div>
        </div>
    );
});
...
function Screen({ visible, children, className = '' }: ScreenProps) {
    return (
        <div className={`screen ${visible ? 'visible' : ''} ${className}`}>
            {children}
        </div>
    );
}
...
const StartScreen = memo(function StartScreen({ visible, onStart }: StartScreenProps) {
    return (
        <Screen visible={visible}>
            ...
            <p className="desc">
                Start as a larva. Eat to evolve.<br />
 ...
 */
const GameOverScreen = memo(function GameOverScreen({ visible, finalScore, onRestart }: GameOverScreenProps) {
    return (
        <Screen visible={visible} className="game-over">
            <h2 className="title game-over-title">Consumed</h2>
            <p className="desc">
                You returned to the depths.<br />
...
export function Game() {
    ...
    const [rank, setRank] = useState(LEVELS[0].title);
    ...
    const showNotification = useCallback((level: Level) => {
        ...
        setRank(level.title);
        setNotification({ title: level.title, visible: true });

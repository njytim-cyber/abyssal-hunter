...
export class Entity {
    ...
    private getColorForType(type: EntityType): string {
        switch (type) {
            case 'food': return CONFIG.colors.food;
            case 'prey': return CONFIG.colors.prey;
            case 'predator': return CONFIG.colors.predator;
        }
    }
    ...
    update(playerX: number, playerY: number, frame: number): void {
        ...
        if (this.x > CONFIG.worldSize) { this.x = CONFIG.worldSize; this.vx *= -1; }
        if (this.y < 0) { this.y = 0; this.vy *= -1; }
        if (this.y > CONFIG.worldSize) { this.y = CONFIG.worldSize; this.vy *= -1; }
    }

...
export class GameEngine {
    ...
    private spawnEntities(): void {
        ...
        // Spawn enemies (only prey during spawn protection)
        const enemyCount = this.entities.filter(e => e.type !== 'food').length;
        if (enemyCount < CONFIG.spawn.maxEnemies) {
            // During spawn protection, only spawn prey so player can grow
            const isPredator = this.spawnProtection <= 0 && Math.random() > 0.6;
            let radius: number;
            let type: EntityType;

            if (isPredator) {
                radius = this.player.radius * (1.2 + Math.random() * 0.5);
                type = 'predator';
            } else {
                radius = Math.max(10, this.player.radius * (0.2 + Math.random() * 0.6));
                type = 'prey';
            }

            // Find valid spawn position (away from player)
            let ex: number = 0, ey: number = 0;
            let attempts = 0;
    ...
    private updatePhysics(): void {
        ...
        let inputActive = this.input.active || this.keys.dash;
        ...
        for (let i = this.entities.length - 1; i >= 0; i--) {
            ...
            if (d < playerRadius + e.radius) {
                if (playerRadius > e.radius) {
                    ...
                    // Spawn particles
                    this.particlePool.spawnBurst(e.x, e.y, 8, e.color, 4);
        ...
        if (this.playerDashing && this.frame % 3 === 0) {
            this.particlePool.spawnBurst(this.player.x, this.player.y, 3, CONFIG.colors.ink, 6);
    ...
    private checkLevelUp(): void {
        ...
        if (nextLevel && this.player.radius >= nextLevel.size) {
    ...
    private render(): void {
        ...
        this.camera.x = Math.max(0, Math.min(this.camera.x, CONFIG.worldSize - this.width / this.camera.scale));
        this.camera.y = Math.max(0, Math.min(this.camera.y, CONFIG.worldSize - this.height / this.camera.scale));
